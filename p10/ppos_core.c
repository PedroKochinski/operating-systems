...
int sem_down(semaphore_t *s) {
    printf("\033[4;33m\033[3msem_down %d\n\033[0m", curr->id);
    if (s) {
        enter_cs(&lock);
        s->counter--;
        leave_cs(&lock);
...
int sem_up(semaphore_t *s) {
    ...
    if (s) {
        ...
        leave_cs(&lock);
        if (s->counter <= 0) {
            task_resume(s->semaphore_queue, &task_queue);  // se há espaço, resume a tarefa que pediu o down do semáforo
        }
        return 0;
    } else {
        return -1;
...
int sem_destroy(semaphore_t *s) {
    if (s) {
        while (s->semaphore_queue) {
            task_resume(s->semaphore_queue, &task_queue);
        }
        return 0;
    } else {
        return -1;
...
#endif
    printf("\033[4;33m\033[3mtask_suspend %d\n\033[0m", curr->id);
