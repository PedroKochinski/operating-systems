...
int sem_down(semaphore_t *s) {
    ...
    if (s) {
        ...
        s->counter--;
        leave_cs(&lock);
        if (s->counter < 0) {  // se não há mais espaço, suspend a tarefa que pediu o down do semáforo
#ifdef DEBUG
            printf("\033[4;33m\033[3mnot enough space in semaphore requestet by %d\n\033[0m", curr->id);
#endif
...
int sem_up(semaphore_t *s) {
    ...
    if (s) {
        ...
        if (s->semaphore_queue)
            printf("sem_queue first task %d \n", s->semaphore_queue->id);
        else
            printf("sem_queue is empty\n");
#endif
        if (s->counter <= 0) {
            if (s->semaphore_queue) task_resume(s->semaphore_queue, &s->semaphore_queue);  // se há espaço, resume a tarefa que pediu o down do semáforo
        }
        return 0;
    } else {
        return -1;
    ...
    if (suspended_queue != NULL) {
        ...
        for (int i = 0; i < queue_size((queue_t *)suspended_queue); i++) {
            if (aux2->wake_up_time <= systime() && aux2->wake_up_time != -1) {
                task_resume(aux2, &suspended_queue);
            }
            aux1 = aux1->next;
            aux2 = aux1;
        }
...
void task_sleep(int t) {
    // printf("\033[4;33m\033[3mtask_sleep %d\n\033[0m", curr->id);
    if (t > 0) {
...
#endif
    if (task) {
        // printf("task_wait: curr id: %d wait id: %d\n", curr->id, task->id);
...
#endif
    // printf("\033[3;34m    Task %d created\033[0m\n", pCounter);
    ucontext_t context;
    char *stack;
    stack = malloc(STACKSIZE);
    getcontext(&context);
...
void task_exit(int exit_code) {
    ...
    if (queue_size((queue_t *)suspended_queue) > 0) {
        ...
        for (int i = 0; i < queue_size((queue_t *)suspended_queue); i++) {
            if (aux->waiting_id == curr->id) task_resume(aux, &suspended_queue);
            aux = aux->next;
        }
    }
